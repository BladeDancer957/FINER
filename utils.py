...
class Conloss_proposal(nn.Module):
    ...
    def __init__(self, opts=None, temperature=0.07):
    ...
    def forward(self, pre_logits, pre_logits_prev, proposal):
        """
        :pre_logits : feature with shape (bsz,seq_len,hidden_dim)
        :pre_logits_prev : feature with shape (bsz,seq_len,hidden_dim)
        :proposal: bsz,seq_len
        :return loss
        """

        bsz, seq_len, hidden_dim = pre_logits.shape
        assert pre_logits.shape == pre_logits_prev.shape

        proposal = proposal.view(-1)  # bsz*seq_len
        # mask without dummy proposal
        mask_undummy = (proposal >= 0) & (proposal < self.opts.num_types)

        pre_logits = pre_logits.view(-1, hidden_dim)
        pre_logits_prev = pre_logits_prev.view(-1, hidden_dim)
...
def save_predicts_to_txt(x_list, y_list, pred_list, label_list, pred_file_name='pred_file.txt', pad_token_label_id=-100, model_name='bert-base-cased'):
    '''
