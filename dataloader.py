...
def read_ner(datapath, phase, label_list):
    if isinstance(datapath,list):
        if len(datapath)>1 and phase!="train":
            print("In %s phase, more than one domain data are combined!!!"%(phase))
...
class NER_dataloader():
    def __init__(self, data_path, domain_name, batch_size, entity_list=[], n_samples=-1, is_filter_O=False, schema='BIO', is_load_disjoin_train=False,sample_ratio=0.6,incremental_method="OURS"):
        ...
        if entity_list=="" or entity_list==[]:
            print('Loading the default entity list from domain %s...'%domain_name)
            self.entity_list = domain2entity[domain_name]
        else:
            print('Loading the pre-defined entity list...')
        ...
        print('label_list = %s'%str(self.label_list))

        # Load data
        print("Load training set data")

        if self.is_load_disjoin_train:
            if isinstance(data_path,list):
                train_data_ckpt = os.path.join(data_path[0],'train_fg_%d_pg_%d.pth'%(params.FG,params.PG))
            else:
                train_data_ckpt = os.path.join(data_path,'train_fg_%d_pg_%d.pth'%(params.FG,params.PG))
   
            print('Loading data from %s...'%train_data_ckpt)
            ...
            print("train size for each split: %s" % 
            ...
            print("train size: %d" % (len(inputs_train)))
        ...
        print("Load development set data")
        ...
        print("label distribution for dev set")
        print(sorted(get_label_distribution(y_dev,self.label_list,count=True).items(),key=lambda x:x[0]))
        print("label distribution for test set")
        print(sorted(get_label_distribution(y_test,self.label_list,count=True).items(),key=lambda x:x[0]))
        print("dev size %d; test size: %d;" % (len(inputs_dev), len(inputs_test)))
    ...
    def get_dataloader(self, first_N_classes=-1, select_entity_list=[], phase=['train','dev','test'], is_filter_O=None, filter_entity_list=[], is_ground_truth_train=False, reserved_ratio=0.0):
        ...
        #select_entity_list_length = len(select_entity_list)
        # if 'train' in phase:
        #     if select_entity_list_length>1:
        #         select_entity_list = random.sample(select_entity_list, math.ceil(select_entity_list_length/2))
        #         sample_ratio = self.sample_ratio2
        #     else:
        #         sample_ratio = self.sample_ratio1
        ...
        if first_N_classes!=-1 or select_entity_list!=[]:
            ...
            print("Select all classes for training !")
        ...
        if is_filter_O:
            ...
            print('Filter out all sentences contains ONLY O labels...')
            if 'train' in phase:
                ...
                print('retain_ratio_train=%d%%'%(retain_ratio_train))
            ...
            if 'dev' in phase:
                ...
                print('retain_ratio_dev=%d%%'%(retain_ratio_dev))
            ...
            if 'test' in phase:
                ...
                print('retain_ratio_test=%d%%'%(retain_ratio_test))
                inputs_test, y_test = filter_O_inputs_test, filter_O_y_test

        if 'train' in phase:
            
            num_elements_to_choose = int(len(y_train) * self.sample_ratio)
            random_combined = random.sample(list(zip(inputs_train, y_train)), num_elements_to_choose)
            inputs_train, y_train = zip(*random_combined)

            dataset_train = Dataset(inputs_train, y_train)
            dataloader_train = DataLoader(dataset=dataset_train, 
                                        batch_size=self.batch_size, 
                                        shuffle=True if self.incremental_method!="CFNER" else False, 
...
if __name__ == "__main__":
    #pass
    spilt_dataset_random(['datasets/NER_data/ontonotes5'], 'train', domain2entity['ontonotes5'], 10, 1, 'BIO')
